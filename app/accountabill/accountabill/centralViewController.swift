...
class centralViewController: UIViewController, EmbeddedViewControllerDelegate {
    ...
    var shouldShowBottomViewController = false
    var shouldShowLeftViewController = true
    var shouldShowRightviewController = true
    
    fileprivate let swipeAnimateDuration = 0.4
    ...
    // MARK: - Containers
    fileprivate func showContainer(_ position: SwipePosition) {
    ...
    // Append embedded view to container view's view hierachy
    func addEmbeddedViewController(_ viewController: UIViewController, position: SwipePosition) {
        (viewController as! EmbeddedViewControllerReceiver).embeddedDelegate = self
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if let
            ...
            let embeddedViewController = navController.topViewController as? EmbeddedViewControllerReceiver {
            embeddedViewController.embeddedDelegate = self
        }
            
            // for EmbeddedViewControllers are NOT embedded in UINavigationControllers
        else if let embeddedViewController = segue.destination as? EmbeddedViewControllerReceiver {
            embeddedViewController.embeddedDelegate = self
        }
    }
}
